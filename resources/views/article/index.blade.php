                       ...
                       @foreach ($articles as $article)
                           <li class="border-2 my-2">
                                <div class="flex flex-row flex-auto justify-between">
                                    ...
                                    <div>
                                        <form action="/dashboard/articles/{{$article->id}}" method="post" class="inline-block">
                                            @method('DELETE')
                                            @csrf
                                            <button type="submit" class="rounded bg-red-500 text-sm py-1 px-1">hapus</button>
                                        </form>
                                        <a href="/dashboard/articles/{{$article->id}}/edit" class="rounded bg-red-500 text-sm py-1 px-1">edit</a>
                                    </div>
